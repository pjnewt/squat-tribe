...
function loadState(){
  try {
    const raw = JSON.parse(localStorage.getItem('squatTribeV165')||'{}');
...
function saveState(){ state.sessions = normalizeSessions(state.sessions); localStorage.setItem('squatTribeV165', JSON.stringify(state)); }
...
function logSession(){
  ...
  state.sessions.push(normalizeSession({ id:`${Date.now()}-${Math.random().toString(36).slice(2,7)}`, date:new Date().toISOString(), exerciseId:d.exerciseId, exerciseName:d.exerciseName, exerciseType:d.exerciseType, ds:d.ds, totalDS:d.ds, mls:d.mls, anchorReps:d.anchorReps, myoSets:d.myoSets, externalLoad:d.externalLoad, bodyweight:Number(state.user.bodyweight || 0), totalActiveTime:d.totalActiveTime, totalReps:d.totalReps, leftDS:d.leftDS ?? null, rightDS:d.rightDS ?? null, balanceDifference:d.balanceDifference ?? null, leftTotalReps:d.leftTotalReps ?? null, rightTotalReps:d.rightTotalReps ?? null }));
