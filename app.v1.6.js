...
function renderMyo(){
  ...
  if (phase === 'ready') {
    ...
    phaseCard = `<div class="card center-text"><div class="title" style="font-size:26px">Myo set in progress</div><div class="small" style="margin-top:10px">Set timer</div><div class="metric big-metric">${formatTime(data.myoCurrentTime)}</div></div>`;
    ...
    phaseCard = `<div class="card"><div class="title" style="font-size:26px">Record Myo reps</div><p class="subtitle">The set has been timed. Record the reps you completed for this Myo set.</p><label style="display:block;margin-top:12px">Completed reps<input id="myo-reps" type="number" min="0" step="1"></label><div class="small" style="margin-top:10px">Set duration: ${formatTime(data.myoCurrentTime)}</div></div>`;
