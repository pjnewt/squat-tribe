...
function speakCue(text){
  if (!state?.user?.voiceCues) return;
  try {
...
const defaultState = {
  onboarded: false,
  onboardingIndex: 0,
  user: { bodyweight:'', defaultLoad:'0', sensorSensitivity:'high', sensorCheckedAt:null, sensorReady:false, voiceCues:true },
  rotationMode:'fixed',
  pointer:0,
  sessions:[],
  redFlag:false,
...
function loadState(){
  try {
    const raw = JSON.parse(localStorage.getItem('squatTribeV169')||'{}');
...
function saveState(){ state.sessions = normalizeSessions(state.sessions); localStorage.setItem('squatTribeV169', JSON.stringify(state)); }
  ...
  app.innerHTML = `<div class="screen">${nav('Profile Setup')}
    <div class="card"><div class="title">Your profile</div><p class="subtitle">These values are used for Mechanical Load Score and Density Score.</p>
      <div class="grid" style="margin-top:14px">
        ...
        <label>Voice cues<select id="voiceCues"><option value="on" ${state.user.voiceCues!==false?'selected':''}>On</option><option value="off" ${state.user.voiceCues===false?'selected':''}>Off</option></select></label>
      </div>
    </div>
...
function finishAnchor(){
  stopAnchorTimer(); speakCue('Stop'); const data = activeSessionData();
  const mirrored = isMirroredSecondLeg();
...
function startRest(seconds, type='breath'){
  clearInterval(restTimer); stopMyoTicker(); state.draft.restRemaining = seconds; state.draft.restType = type; state.view = 'rest'; speakCue('Rest');
  restTimer = setInterval(()=>{ state.draft.restRemaining -= 1; if (state.draft.restRemaining <= 0) { clearInterval(restTimer); try { navigator.vibrate?.([90,40,90]); } catch {} state.draft.myoState = 'ready'; state.view = 'myo'; const count = (activeSessionData()?.myoSets?.length || 0) + 1; const words = ['one','two','three','four','five','six','seven','eight']; speakCue(count===1 ? 'Myo one' : ('Myo ' + (words[count-1] || count))); } render(); },1000);
  render();
}
function startMyoSet(){ const data = activeSessionData(); const mt = currentMirroredMyoTarget(); if (mt != null) data.myoTarget = mt; state.draft.myoState = 'active'; speakCue('Start'); startMyoTicker(); render(); }
function stopMyoSet(){ stopMyoTicker(); speakCue('Stop'); const data = activeSessionData(); if (!data.myoCurrentTime) data.myoCurrentTime = 10; state.draft.myoState = 'record'; render(); }
function logMyoSet(){
...
function prepareSummary(autoEnded){ clearInterval(restTimer); stopAnchorTimer(); stopMyoTicker(); state.draft.autoEnded = autoEnded; state.view = 'summary'; speakCue('Complete'); render(); }
...
function triggerInstall(){
  ...
  state.installHelpOpen = true; alert('Open the page in Chrome, tap the three-dot menu, then choose Add to Home screen or Install app.'); render();
}
...
app.addEventListener('click', (e)=>{
  ...
  if (action === 'save-settings') { state.user.bodyweight = document.getElementById('bodyweight').value; state.user.defaultLoad = document.getElementById('defaultLoad').value || '0'; state.user.sensorSensitivity = document.getElementById('sensorSensitivity').value; state.user.voiceCues = document.getElementById('voiceCues').value === 'on'; state.rotationMode = document.getElementById('rotationMode').value; state.onboarded = true; state.view = 'home'; return render(); }
