...
function loadState(){
  try {
    const raw = JSON.parse(localStorage.getItem('squatTribeV164')||'{}');
    return { ...defaultState, ...raw, sessions: withIds(raw.sessions), user:{...defaultState.user, ...(raw.user||{})} };
...
function saveState(){ localStorage.setItem('squatTribeV164', JSON.stringify(state)); }
...
function renderAnchor(){
  ...
  app.innerHTML = `<div class="screen">${nav('Anchor Set')}
    ...
    <div class="card"><div class="title" style="font-size:24px">Record reps</div>
      ...
      <div class="adjust-row"><button class="btn secondary mini-btn" data-action="anchor-minus">−</button><div class="small center-text">Use these buttons to correct the rep total if needed.</div><button class="btn secondary mini-btn" data-action="anchor-plus">+</button></div>
    </div>
...
function renderSideTransition(){
  ...
  app.innerHTML = `<div class="screen">${nav('Leg Transition')}
    <div class="card transition-card"><div class="title">${sidePretty(s.completedSide || 'left')} complete</div><p class="subtitle">The weaker side sequence is finished. Review the first-leg density score, then continue to the opposite leg.</p>
      ...
      <div class="small" style="margin-top:12px">The app will now run the full Anchor / Rest / Myo sequence for the ${sidePretty(nextSide).toLowerCase()}.</div>
    </div>
...
function finishAnchor(){
  ...
  data.totalActiveTime = data.anchorTime; data.myoTarget = Math.max(3, Math.round(data.anchorReps * 0.25));
...
function startSecondLegSetup(){ const d = state.draft; d.currentSide = d.sideOrder[1]; d.sideTransition = true; d.myoState = 'ready'; state.view = 'sessionSetup'; render(); }
...
function completeCurrentSide(autoEnded){
  ...
  if (d.currentSide === d.sideOrder[0]) {
    d.sideTransitionSummary = { completedSide, ds: sideData.ds, totalReps: sideData.totalReps };
...
function startMyoSet(){ state.draft.myoState = 'active'; startMyoTicker(); render(); }
...
function logSession(){
  ...
  state.sessions.push({ id:`${Date.now()}-${Math.random().toString(36).slice(2,7)}`, date:new Date().toISOString(), exerciseId:d.exerciseId, exerciseName:d.exerciseName, ds:d.ds, mls:d.mls, anchorReps:d.anchorReps, myoSets:d.myoSets, externalLoad:d.externalLoad, totalActiveTime:d.totalActiveTime, leftDS:d.leftDS || null, rightDS:d.rightDS || null, balanceDifference:d.balanceDifference || null, leftTotalReps:d.leftTotalReps || null, rightTotalReps:d.rightTotalReps || null });
...
function updateRedFlag(id){ const related = state.sessions.filter(s=>s.exerciseId===id).slice(-7); if (related.length < 3) { state.redFlag = false; return; } const last3 = related.slice(-3); const avg = related.reduce((a,b)=>a+b.ds,0) / related.length; state.redFlag = last3.every(s=>s.ds < avg * 0.8); }
function renderHistory(){
  const rows = [...state.sessions].reverse().map(s=>`<div class="history-row ${state.historyEdit?'checkbox-row':''}">${state.historyEdit?`<input type="checkbox" data-session-id="${s.id}" ${state.historySelected.includes(s.id)?'checked':''}>`:''}<div style="flex:1"><div>${s.exerciseName}</div><div class="small">${new Date(s.date).toLocaleString()}</div>${s.balanceDifference!=null?`<div class="small">Side diff ${s.balanceDifference.toFixed(1)}%</div>${symmetryMarkup(s.leftDS||0,s.rightDS||0)}`:''}</div><div class="metric">DS ${s.ds.toFixed(1)}</div></div>`).join('') || '<div class="small">No sessions logged yet.</div>';
