...
function loadState(){
  try { return {...defaultState, ...JSON.parse(localStorage.getItem('squatTribeV16')||'{}')}; }
  catch { return {...defaultState}; }
}
let state = loadState();
if (state.onboarded && state.view === 'onboarding') state.view = 'home';

function saveState(){ localStorage.setItem('squatTribeV16', JSON.stringify(state)); }
...
function makeSideData(){
  return { externalLoad:0, anchorTime:0, anchorReps:0, myoTarget:0, myoSets:[], myoSetTimes:[], strikes:0, totalActiveTime:0, totalReps:0, ds:0, mls:0, myoCurrentTime:0, liveRepCount:0, sensorAvailable:true, sensorMessage:'' };
}
...
function renderAnchor(){
  ...
  const anchorValue = sideData.anchorReps || sideData.liveRepCount || '';
  app.innerHTML = `<div class="screen">${nav('Anchor Set')}
    ...
    <div class="card center-text"><div class="small">Anchor duration</div><div class="metric big-metric">${formatTime(sideData.anchorTime)}</div>
      <div class="small" style="margin-top:10px">Live reps</div><div id="live-anchor-reps" class="metric" style="font-size:30px">${sideData.liveRepCount || 0}</div>
      <div class="small" style="margin-top:8px">${sideData.sensorMessage || 'Hold the phone in both hands and move steadily.'}</div>
    ...
    <div class="card"><div class="title" style="font-size:24px">Record reps</div>
      <label style="display:block;margin-top:12px">Anchor reps<input id="anchor-reps" type="number" min="0" step="1" value="${anchorValue}"></label>
      <div class="small" style="margin-top:8px">The live counter fills this automatically. Adjust it if needed.</div>
    </div>
...
function renderMyo(){
  ...
  if (phase === 'ready') {
    ...
    phaseCard = `<div class="card center-text"><div class="title" style="font-size:26px">Myo set in progress</div><div class="small" style="margin-top:10px">Set timer</div><div class="metric big-metric">${formatTime(data.myoCurrentTime)}</div><div class="small" style="margin-top:10px">Live reps</div><div id="live-myo-reps" class="metric" style="font-size:30px">${data.liveRepCount || 0}</div><div class="small" style="margin-top:8px">${data.sensorMessage || 'Hold the phone in both hands and move steadily.'}</div></div>`;
    ...
    phaseCard = `<div class="card"><div class="title" style="font-size:26px">Record Myo reps</div><p class="subtitle">The set has been timed. Record the reps you completed for this Myo set.</p><label style="display:block;margin-top:12px">Completed reps<input id="myo-reps" type="number" min="0" step="1" value="${data.liveRepCount || ''}"></label><div class="small" style="margin-top:10px">Set duration: ${formatTime(data.myoCurrentTime)}</div><div class="small" style="margin-top:8px">The live counter fills this automatically. Adjust it if needed.</div></div>`;
...
function stopRepSensor(){
  try { repSensor?.stop?.(); } catch {}
  repSensor = null;
}

function activeLiveData(){
  return state.draft?.exerciseType === 'unilateral' ? currentSideData() : state.draft;
}

async function startRepSensor(mode='anchor'){
  const data = activeLiveData();
  if (!data) return false;
  stopRepSensor();
  data.liveRepCount = 0;
  data.sensorMessage = 'Tracking reps…';
  repSensor = createRepSensor((count)=>{
    const target = activeLiveData();
    if (!target) return;
    target.liveRepCount = count;
    const el = mode === 'anchor' ? document.getElementById('live-anchor-reps') : document.getElementById('live-myo-reps');
    if (el) el.textContent = String(count);
  });
  const ok = await repSensor.start();
  data.sensorAvailable = ok;
  if (!ok) data.sensorMessage = 'Motion permission unavailable. Record reps manually.';
  render();
  return ok;
}

function createRepSensor(onRep){
  let repCount = 0;
  let running = false;
  let lastState = 'idle';
  let lastRepAt = 0;
  ...
  async function start(){
    ...
    motionHandler = (event) => {
      ...
      if (!acc) return;
      const samples = [acc.x, acc.y, acc.z].filter(v => typeof v === 'number');
      if (!samples.length) return;
      const axis = samples.reduce((a,b)=>Math.abs(b) > Math.abs(a) ? b : a, 0);
      smooth = smooth * 0.8 + axis * 0.2;
      const now = Date.now();
...
function stopAnchorTimer(){ clearInterval(anchorTimer); if (state.draft) state.draft.anchorRunning = false; }
...
function stopMyoTicker(){ clearInterval(myoTimer); }
...
function finishAnchor(){
  ...
  const d = state.draft;
  const data = activeSessionData();
  data.anchorReps = Number(document.getElementById('anchor-reps')?.value || 0);
...
function startMyoSet(){
  ...
  activeSessionData().liveRepCount = 0;
  startMyoTicker();
  startRepSensor('myo');
  render();
}
...
app.addEventListener('click', (e)=>{
  ...
  if (action === 'back') {
    ...
    else if (['sessionSetup','anchor','rest','myo','summary'].includes(state.view)) { clearInterval(restTimer); stopAnchorTimer(); stopMyoTicker(); stopRepSensor(); state.view='home'; }
    return render();
  }
  ...
  if (action === 'anchor-start-clock') { state.draft.anchorRunning = true; activeLiveData().liveRepCount = 0; startAnchorTicker(); startRepSensor('anchor'); return render(); }
  if (action === 'anchor-stop-clock') { stopAnchorTimer(); stopRepSensor(); return render(); }
  ...
  if (action === 'start-myo-set') return startMyoSet();
